...
while True:
    ...
    if len(keypoint_results) > 0:
        ...
        # 当手势分类标签检测到 "first" 时，模拟鼠标按下并拖动
        if gesture_class == 'fist':
            if not mouse_pressed:
                mouse_pressed = True  # 标记为鼠标按下
                # 获取关键点数据（关键点的坐标）
                keypoints = keypoint_results[0].keypoints[0]
                for i, point in enumerate(keypoints.xy[0]):
                    if i == 13:  # 使用中指根部作为拖动的起点
                        mapped_x = int(point[0] / frame.shape[1] * screen_width)
                        mapped_y = int(point[1] / frame.shape[0] * screen_height)
                        pyautogui.mouseDown(mapped_x, mapped_y)  # 模拟鼠标按下
                        start_pos = (mapped_x, mapped_y)  # 记录鼠标按下位置
                        break
        ...
        if mouse_pressed and start_pos:
            ...
            for i, point in enumerate(keypoints.xy[0]):
                if i == 13:  # 继续使用中指根部作为鼠标拖动的参考
                    ...
                    mapped_y = int(point[1] / frame.shape[0] * screen_height)
                    # 计算鼠标的移动距离
                    pyautogui.moveTo(mapped_x, mapped_y)  # 移动鼠标到新位置
                    break
